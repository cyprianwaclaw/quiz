                    ...
                    <div class="pt-2 block">
                        ...
                        @foreach(auth()->user()->invited as $user)
                            @if($user->activePlanSubscriptions()->count())
                                <div class="pl-52 block font-bold">{{$user->name}} PREMIUM</div>
                            @else
                                <div class="pl-52 block">{{$user->name}}</div>
                            @endif
                        @endforeach
                    </div>
                </div>
                        ...
                        @foreach($plans as $plan)
                            <div class="p-4 xl:w-1/2 md:w-1/2 w-full">
                                <div
                                    ...
                                    <h2 class="text-sm tracking-widest title-font mb-1 font-medium">{{$plan->description}}</h2>
                                    <h1 class="text-5xl text-gray-900 leading-none flex items-center pb-4 mb-4 border-b border-gray-200">
                                        <span>{{$plan->price}} {{$plan->currency}}</span><span
                                            class="text-lg ml-1 font-normal text-gray-500">/ {{$plan->invoice_interval}}</span>
                                    </h1>
                                    @foreach($plan->features as $feature)
                                        <p class="flex items-center text-gray-600 mb-2">
                                    <span
                                        class="w-4 h-4 mr-2 inline-flex items-center justify-center bg-green-600 text-white rounded-full flex-shrink-0">
                                    ...
                                    <form method="POST" action="{{ route('buy-plan') }}">
                                        ...
                                        <button
                                            class="flex items-center mt-auto text-white bg-indigo-400 border-0 py-2 px-4 w-full focus:outline-none hover:bg-indigo-500 rounded"
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div
                    ...
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <div class="block text-white text-xl font-bold text-center pb-3">Quizy do akceptacji:</div>
                    </div>
        ...
        <div class="pb-4">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div
                    class="sm:rounded-lg bg-gradient-to-r bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 p-2 sm:p-6 overflow-hidden">
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <div class="block text-white text-xl font-bold text-center pb-3">Ostanie wypłaty:</div>
                    </div>
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead
                            class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{route('payout.show', $payout->id)}}"
                                       class="items-center mt-auto text-white bg-indigo-400 border-0 py-2 px-4 w-full focus:outline-none hover:bg-indigo-500 rounded">Szczegóły</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
